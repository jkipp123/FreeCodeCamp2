...
suite('Functional Tests', () => {
  ...
  suite('POST to /api/check', () => {
    ...
    test('Required Field(s) Missing', done => {
      ...
      chai.request(server)
        .post(`/api/check`)
        .send({ puzzle: input, coordinate: coordinate, value: '' })
        .end(function(err, res) {
          assert.equal(res.status, 200);
          assert.property(res.body, 'error');
          assert.deepEqual(res.body, error);
    ...
    test('Coordinate Out of Bounds', done => {
      ...
      Promise.all([
        requester.post(`/api/check`).send({ puzzle: input, coordinate: coordinate1, value: value }),
        requester.post(`/api/check`).send({ puzzle: input, coordinate: coordinate2, value: value }),
      ])
        .then((responses) => {
          try {
